...
const StudentRegistration = () => {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-br from-gray-900 to-blue-900 p-4">
  <div className="w-full max-w-md bg-gray-800 rounded-xl shadow-xl p-8 transform transition-all duration-300 hover:shadow-2xl border border-gray-700">
    <h2 className="text-2xl font-bold text-center text-gray-100 mb-6 relative pb-3 after:content-[''] after:absolute after:bottom-0 after:left-1/2 after:-translate-x-1/2 after:w-12 after:h-1 after:bg-gradient-to-r after:from-blue-500 after:to-indigo-400 after:rounded-full">Student Registration</h2>
    
    {submitMessage && (
      <div className={`p-4 mb-6 rounded-lg text-center text-sm font-medium ${submitMessage.includes('successful') ? 'bg-green-900 text-green-300 border border-green-700' : 'bg-red-900 text-red-300 border border-red-700'}`}>
        {submitMessage}
      </div>
    )}
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        ...
        <input
          ...
          className={`w-full px-4 py-2.5 rounded-lg bg-gray-700 text-white focus:ring-2 focus:ring-offset-1 focus:ring-offset-gray-800 transition duration-200 ${errors.name ? 'border-red-500 focus:ring-red-400' : 'border-gray-600 focus:border-blue-500 focus:ring-blue-400'}`}
      ...
      <div>
        ...
        <input
          ...
          onChange={handleChange}
          className={`w-full px-4 py-2.5 rounded-lg bg-gray-700 text-white focus:ring-2 focus:ring-offset-1 focus:ring-offset-gray-800 transition duration-200 ${errors.rollNumber ? 'border-red-500 focus:ring-red-400' : 'border-gray-600 focus:border-blue-500 focus:ring-blue-400'}`}
          placeholder="Enter your roll number"
        />
        {errors.rollNumber && <p className="mt-1.5 text-sm text-red-400 font-medium">{errors.rollNumber}</p>}
      </div>
      ...
      <div>
        ...
        <input
          ...
          onChange={handleChange}
          className={`w-full px-4 py-2.5 rounded-lg bg-gray-700 text-white focus:ring-2 focus:ring-offset-1 focus:ring-offset-gray-800 transition duration-200 ${errors.email ? 'border-red-500 focus:ring-red-400' : 'border-gray-600 focus:border-blue-500 focus:ring-blue-400'}`}
      ...
      <div>
        ...
        <input
          ...
          onChange={handleChange}
          className={`w-full px-4 py-2.5 rounded-lg bg-gray-700 text-white focus:ring-2 focus:ring-offset-1 focus:ring-offset-gray-800 transition duration-200 ${errors.phoneNumber ? 'border-red-500 focus:ring-red-400' : 'border-gray-600 focus:border-blue-500 focus:ring-blue-400'}`}
          placeholder="Enter your 10-digit phone number"
        />
        {errors.phoneNumber && <p className="mt-1.5 text-sm text-red-400 font-medium">{errors.phoneNumber}</p>}
      </div>
      
      <div>
        <label className="block text-sm font-medium text-gray-300 mb-3">Select Clubs (Multiple Selection Allowed)</label>
        <div className="space-y-2 bg-gray-700 p-4 rounded-lg border border-gray-600">
          {clubs.map(club => (
            <div key={club.id} className="mb-4 border-b border-gray-600 pb-4 last:border-b-0 last:pb-0 hover:bg-gray-600 rounded-md transition-colors duration-200">
              <div className="flex items-center mb-2">
                <input
                  ...
                  className="h-5 w-5 text-blue-500 focus:ring-blue-400 border-gray-500 rounded cursor-pointer bg-gray-600"
                />
                <label htmlFor={`club-${club.id}`} className="ml-2 block text-sm font-medium text-gray-200 cursor-pointer">
                  {club.name} - {club.description}
                </label>
              </div>
              {formData.selectedClubs.includes(club.id.toString()) && (
                <div className="ml-7 mt-3 animate-fadeIn">
                  <p className="text-sm font-medium text-gray-300 mb-2">Select SIGs for {club.name}:</p>
                  <div className="space-y-2 bg-gray-800 p-3 rounded-lg border border-gray-600 max-h-48 overflow-y-auto shadow-sm">
                    {(clubSigs[club.id] || []).map(sig => (
                      <div key={sig.id} className="flex items-center p-2 hover:bg-gray-700 rounded-md transition-colors duration-200">
                        <input
                          type="checkbox"
                          ...
                          className="h-4 w-4 text-indigo-500 focus:ring-indigo-400 border-gray-500 rounded cursor-pointer bg-gray-600"
                        />
        ...
        {errors.selectedClubs && <p className="mt-2 text-sm text-red-400 font-medium">{errors.selectedClubs}</p>}
        {errors.selectedSigs && <p className="mt-2 text-sm text-red-400 font-medium">{errors.selectedSigs}</p>}
      </div>
      
      <button
        type="submit"
        disabled={isSubmitting}
        className={`w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-medium py-2.5 px-4 rounded-lg hover:from-blue-700 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 focus:ring-offset-gray-800 transition duration-300 transform hover:-translate-y-0.5 ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''}`}
      >
     ...
     {/* <div className="text-center mt-6">
        <p className="text-sm text-gray-400">
          Already registered? <a href="/login" className="text-blue-400 hover:text-blue-300 font-medium">Login here</a>
        </p>
      </div> */}
    </form>
  </div>
